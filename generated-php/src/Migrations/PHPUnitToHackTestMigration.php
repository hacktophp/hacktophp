...
', $comment_text), function ($line) use($doc_tag) {
    ...
    private function migrateDataProvider(HHAST\MethodishDeclaration $decl)
    {
        ...
        if ($attrs === null) {
            if ($comment_text !== null) {
                $leading = $decl->getFirstTokenx()->getLeading()->replace($comment, $comment->withText($comment_text));
            } else {
                $leading = array();
                foreach ($decl->getFirstTokenx()->getLeading()->getItems() as $item) {
                    if ($item === $comment) {
                        break;
                    }
                    $leading[] = $item;
    ...
    private function renameSetUpTearDownFunctions(HHAST\FunctionDeclarationHeader $node)
    {
        ...
        $new_modifiers = \array_map(function ($m) {
            ...
            return $m->withLeading(HHAST\Missing())->withTrailing(new HHAST\WhiteSpace(' '));
...
')))), $body);
    ...
    private final function migrateExpectExceptionInStatements(array $statements, string $indent)
    {
        ...
        $expect_exception = $statements[$idx]->getExpressionx();
